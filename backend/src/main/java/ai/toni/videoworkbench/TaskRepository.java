...
class TaskRepository {
  ...
  private final RowMapper<TranscriptSegment> segmentMapper = (r, n) -> new TranscriptSegment(r.getLong("id"), r.getLong("start_ms"), r.getLong("end_ms"), r.getString("text"), r.getString("translation"));
  ...
  void replaceSegments(String taskId, List<TranscriptSegment> segments) { jdbc.update("delete from transcript_segments where task_id=?", taskId); for (TranscriptSegment segment : segments) jdbc.update("insert into transcript_segments(task_id,start_ms,end_ms,text,translation) values(?,?,?,?,?)", taskId,segment.startMs(),segment.endMs(),segment.text(),segment.translation()); }
  void replaceTranslations(String taskId, List<TranscriptSegment> segments) { for (TranscriptSegment segment : segments) jdbc.update("update transcript_segments set translation=? where task_id=? and id=?", segment.translation(), taskId, segment.id()); }
